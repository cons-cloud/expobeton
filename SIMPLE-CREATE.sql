-- Script SIMPLE de création - Version minimale qui fonctionne
-- Exécutez ce script après le nettoyage
...
-- 5. Insérer quelques emails de test
INSERT INTO emails_sent (campaign_id, recipient_email, status, user_id)
SELECT 
  c.id,
  'test' || n.num || '@example.com',
  CASE 
    WHEN n.num <= 8 THEN 'delivered'
    WHEN n.num <= 9 THEN 'sent'
    ELSE 'failed'
  END,
  c.user_id
FROM email_campaigns c
CROSS JOIN (SELECT generate_series(1, 10) as num) n
WHERE c.name = 'Newsletter Janvier 2024' 
AND c.user_id = auth.uid()
LIMIT 10;

-- 6. Vérification
SELECT 'CREATION TERMINEE' as info;
